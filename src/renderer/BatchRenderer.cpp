...
BatchRenderer::BatchRenderer(Shader &sh) : shader(sh) {
...
void BatchRenderer::textQuad(float x, float y, float w, float h, float u0,
                             float v0, float u1, float v1, glm::vec4 col) {
...
void BatchRenderer::flush(const glm::mat4 &proj, GLuint textureID) {
  ...
  if (!solidVerts.empty()) {
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, solidVerts.size() * sizeof(Vertex),
                    solidVerts.data());
  ...
  if (!textVerts.empty()) {
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, textVerts.size() * sizeof(Vertex),
                    textVerts.data());
