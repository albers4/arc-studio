...
TextInput::TextInput(float x, float y, float w, float h,
                     const std::string &label, const FontAtlas &font)
  ...
  try {
    if (valueType == Type::String) {
      *static_cast<std::string *>(valuePtr) = editBuffer;
      ...
      value = std::clamp(value, static_cast<int>(minValue),
      ...
      *static_cast<int *>(valuePtr) = value;
      ...
      value = std::clamp(value, static_cast<float>(minValue),
...
void TextInput::onKey(int key, int action, int mods, UIManager &ui) {
  ...
  if (key == GLFW_KEY_RIGHT) {
    if (cursorPos < (int)editBuffer.length())
      cursorPos++;
    if (!shift)
      clearSelection();
    else if (selectionStart == -1)
...
void TextInput::draw(UIManager &ui) {
  ...
  ui.rect(position.x + size.x - m.borderWidth, position.y, m.borderWidth,
  ...
  if (selectionStart != -1 && selectionStart != cursorPos) {
    ...
    if (screenEndX > screenStartX) {
      ui.rect(screenStartX, position.y + 4.0f, screenEndX - screenStartX,
              size.y - 8.0f, glm::vec4(0.3f, 0.5f, 0.8f, 0.8f));
    }
  }
  ...
  if (isFocused && cursorVisible) {
    ...
    cursorScreenX = std::clamp(cursorScreenX, position.x + padding,
                               position.x + size.x - padding);
    ui.rect(cursorScreenX, position.y + 4.0f, 1.0f, size.y - 8.0f,
