...
Slider::Slider(
    ...
    trackColor(0.3f, 0.3f, 0.35f, 1.0f),
    knobColor(0.6, 0.6, 0.7, 1.0f),
    hoverColor(0.8f, 0.8f, 0.9, 1.0f)
{
...
void Slider::update(float mouseX, float mouseY, bool mousePressed) {
    ...
    bool mouseOverKnob = (
        mouseX >= knobLeft && mouseX <= knobRight &&
...
void Slider::draw(UIManager& ui) {
    ...
    ui.rect(knobX - (knobSize / 2), knobY - (knobSize / 2), knobSize, knobSize, finalKnobColor);
